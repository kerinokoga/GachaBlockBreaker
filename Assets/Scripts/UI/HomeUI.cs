...
public class HomeUI : MonoBehaviour
{
    ...
    void ShowHomeCharPopup()
    {
        ...
        void AddVariantRows(string charName)
        {
            foreach (var v in HomeCharManager.Variants)
            {
                ...
                BuildHomeCharRow(contentRt, $"　★ {v.label}", vUnlocked && vHasVideo,
                    vSelected, vSub, () => SelectVariant(vCopy), rowIndex++, rowH, padY);
            }
        }

        // 選択可能な衣装（通常またはバリアント）をひとつでも持つか（並び順用）
        ...
        // デフォルト（セラ）行＋セラのバリアント（二つ名はキャラデータから取得）
        ...
        foreach (var cd in all)
        {
            ...
            string sub = !unlocked ? HomeCharManager.UnlockConditionText(cd)
                       : !hasVideo ? "アニメ準備中" : "";
            BuildHomeCharRow(contentRt, $"{dispName}（{cd.rarity}）",
                unlocked && hasVideo, selected == name, sub,
